...
// Code2SessionContext 平台代发起-起登录凭证校验。
func (auth *Auth) Code2SessionContext(ctx context2.Context, jsCode string) (result ResCode2Session, err error) {
	at, err := auth.GetComponentAccessToken()
	if err != nil {
		return
	}

	var response []byte
	if response, err = util.HTTPGetContext(ctx, fmt.Sprintf(code2SessionURL, auth.appID, jsCode, auth.AppID, at)); err != nil {
		return
	}
...
func (auth *Auth) CheckEncryptedDataContext(ctx context2.Context, encryptedMsgHash string) (result RspCheckEncryptedData, err error) {
	...
	if at, err = auth.GetAuthrAccessToken(auth.appID); err != nil {
		return
	}
